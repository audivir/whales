...
def prepare_mol(mol: Mol, property_name: str, do_charge: bool) -> tuple[Mol, str, bool]:
    ...
    if not do_charge:
        if property_name:
            ...
            mol = Chem.AddHs(mol)
            n_at = mol.GetNumAtoms()
            w = np.ones((n_at, 1), dtype=np.float64)
            w = np.divide(w, n_at, dtype=np.float64)
            w = np.asarray([float(i) for i in w], dtype=np.float64)
